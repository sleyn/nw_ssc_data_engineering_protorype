...
- Production grade bioinformatics pipelines (HPC and Cloud)
- Data solutions co-development with clinical partners (e.g. Roswell Park - NYS CLEP validation).
- Worked in:
  - Russian Academy of Sciences (academy)
  - SBP Medical discovery institute (academy)
  - Agilent Technologies (industry)
...
- `lab_report`, `vitals`, `mrss`, `bal`, `medications`, `skin_biopsies` — 
  structured longitudal event records.
- `demographics`/`ssc_subtype` — static, one row per patient

::: {.finding}
`ssc_subtype.other_dx` — semicolon-separated comorbidity flags (ILD, GERD, PAH).
Every Registry patient's `diagnosis` is SSc regardless of `other_dx` content.
:::
...
Login + data encryption + audit logging gated behind a published demo
account (details in Design Decisions).

[nw-ssc-de.streamlit.app](https://nw-ssc-de.streamlit.app/){.external target="_blank"}
...
height = pd.read_csv("../data/data_v3/demographics.csv")["height"].dropna()
...
for a living adult). All 5 separate `vitals.csv` visits spanning 2017-2020
report ~160.6 lbs for the same patient - likely placeholder.
:::
...
pft = pd.read_csv('../data/data_v3/pft.csv')[["DESCRIPTION", "ORD_VALUE"]]
pft["value"] = pd.to_numeric(pft["ORD_VALUE"], errors="coerce")
targets = [
  "Forced Vital Capacity (% predicted)",
  "Forced Expiratory Volume 1s (% predicted)",
  "Diffusing Capacity CO single breath (% predicted)"
]
pft = pft[pft["DESCRIPTION"].isin(targets)].dropna(subset=["value"])

fig, ax = plt.subplots(figsize=(7, 3.5))
sns.boxplot(
  x="DESCRIPTION", y="value", data=pft, ax=ax, color=NU_PURPLE,
  showfliers=False, boxprops={"facecolor": "none"}
)
sns.stripplot(
  x="DESCRIPTION", y="value", data=pft, ax=ax, color=NU_PURPLE,
  size=4, alpha=0.3, jitter=0.3
)
ax.axhline(40.0, color="#1A1A1A", linestyle="--", linewidth=1)
ax.axhline(130.0, color="#1A1A1A", linestyle="--", linewidth=1)
ax.set_xticklabels(["FVC", "FEV1", "DLCO_SB"])
ax.set_xlabel("")
ax.set_ylabel("% predicted")
ax.set_title("FVC / FEV1 / DLCO_SB (% predicted) — identical 40.0-130.0 bounds")
plt.tight_layout()
plt.show()
```
...
- **Dose**: only 24 distinct strings; a few look like intentionally
  injected errors — negative doses, `999 tablets daily`, `250 g twice daily`
  (likely a mg/g), `500 mL tablets daily` (a volume unit on a solid dosage form)
- **Two distinct, non-overlapping gaps**: 20 rows have dose but no
  medication name; 85 rows have medication name but no dose.
- Medication is normalized on every read (split to cannonical medication name,
...
have `ENTRY_USER_NAME` (a clinician name) - mock PHI.
:::

**Decision:** patient name/DOB suppressed everywhere downstream
(`subject_id` + age-at-event only) — a demonstration of PHI-handling.
`ENTRY_USER_NAME` kept visible: provider-side
...
- Still gives a real "encryption at rest" component to explain: the
  runtime SQLite store the app actually reads from is encrypted
...
Models the **Control Subject / Registry** relationship correctly — cohort
membership, not a foreign-key violation.
...
- Convert raw values to categorical vareables with thresholds
- Make more flexible data visualization (boxplot vs violin)
- Add direct table views.

## Thank you — discussion

Repo: [github.com/sleyn/nw_ssc_data_engineering_protorype](https://github.com/sleyn/nw_ssc_data_engineering_protorype)

App: [nw-ssc-de.streamlit.app](https://nw-ssc-de.streamlit.app/)
