...
- `lab_report`, `vitals`, `mrss` — every row reduces to one
  `(subject, date, value)` triple (which measure varies per row for
  labs/vitals; MRSS has only one possible measure). Same shape, unified
  as one **Observation** abstraction in code (Lab Result / Vital Sign /
  MRSS stay distinct terms in any user-facing view)
- `bal`, `medications`, `skin_biopsies` — structured event records with
  no single value: `bal` has two volumes + site + comment,
  `skin_biopsies` has no numeric value at all (site/indication/specimen/
  image path), `medications` pairs a name with a dose that needed its own
  QC lookup table (not a clean number)
...
- **Two distinct, non-overlapping gaps**: 20 rows have dose but no
  ...
  both blank
- Medication name canonicalized on every read (`data.access`/`data.qc`,
  original preserved in an "as recorded" field) -- the raw `medications`
  table itself still has the 3 separate strings; dose built as a 24-row
  lookup table, both cleaning display and powering the implausibility
  flags
